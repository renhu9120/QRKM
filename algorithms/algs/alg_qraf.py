...
from algorithms.gradient.grad_qraf import grad_qraf
...
from core.hyperparams import QRAF as HP_QRAF
...
def alg_qraf(
    ...
    """Run QRAF with unified float64 quaternion interface.
    ...
    if A_mat.dtype != torch.float64:
        raise TypeError("alg_qraf requires float64 tensors")
    ...
    # Route-A adapter:
    # QRAF internals use q_mat_mul(A, z), while QRKM data use a_k^* z (conjugated rows).
    # Using element-wise conjugated rows keeps QRAF update equations unchanged while
    # matching the forward model used by scripts/main_1_compare_pr_single_trial.py.
    ...
    z_path: List[torch.Tensor] = grad_qraf(
        A_model,
        y,  # pass canonical (n,)
        z0,
        beta=float(HP_QRAF["beta"]),
        T=T,
        x_true=x_true,
        stop_err=stop_err,
        use_pfe=use_pfe,
    ...
    if verbose:
        ...
        for i, z in enumerate(z_iter, start=1):
            log_name = "alg_qraf_pfe" if use_pfe else "alg_qraf"
...
def _smoke_test_main() -> None:
    """Run a single synthetic QRAF trial and save convergence curves."""
    d = 50
    n = 10 * d
    T = 200
    ...
    convergence_csv_name = "qraf_smoke_from_alg_convergence.csv"
    curve_pt_name = "qraf_smoke_from_alg_curve.pt"
    fig_base_name_ln_dist = "qraf_smoke_from_alg_ln_dist_T"
    ...
    if len(x_steps) > 0 and len(ln_vals) > 0:
        ...
        plot_conv_curvs(
            x=x_steps,
            data_series=[
                {"y": np.asarray(ln_vals, dtype=float), "label": "QRAF", "style": "-", "color": "steelblue"},
            ],
            title="QRAF smoke test: ln(dist_T) vs iterations",
