"""Quaternion Accelerated RAF (QARAF) wrapper and smoke-test entry."""
...
from algorithms.gradient.grad_qaraf import grad_qaraf
from algorithms.initialization.init_rsi import init_rsi
...
from core.hyperparams import QARAF as HP_QARAF
...
def alg_qaraf(
    ...
    """Run QARAF with unified float64 quaternion interface.

    Notes
    -----
    `y` must be amplitude measurements `|a^*x|` (not intensity `|a^*x|^2`).
    ...
    if A_mat.dtype != torch.float64:
        raise TypeError("alg_qaraf requires float64 tensors")
    ...
    # --- (1) Initialization ---
    if z0 is None:
        z0 = init_rsi(A_model, y)  # your init returns (d,4) or (d,1,4)
    else:
        z0 = z0.to(device)
        if z0.dim() == 2:
            z0 = z0.unsqueeze(1)
    ...
    z_path: List[torch.Tensor] = grad_qaraf(
        A_model,
        y,  # pass canonical (n,)
        z0,
        T=T,
        x_true=x_true,
        stop_err=stop_err,
        eta=float(HP_QARAF["eta"]),
        beta_i=float(HP_QARAF["beta_i"]),
        beta=float(HP_QARAF["beta"]),
        use_pfe=use_pfe,
    )
    ...
    if verbose:
        ...
        for i, z in enumerate(z_iter, start=1):
            log_name = "alg_qaraf_pfe" if use_pfe else "alg_qaraf"
...
def _smoke_test_main() -> None:
    """Run a single synthetic QARAF trial and save convergence curves."""
    ...
    convergence_csv_name = "qaraf_smoke_from_alg_convergence.csv"
    curve_pt_name = "qaraf_smoke_from_alg_curve.pt"
    fig_base_name_ln_dist = "qaraf_smoke_from_alg_ln_dist_T"
    ...
    # QARAF consumes amplitude measurements psi = |a^*x|, not intensity |a^*x|^2.
    ...
    print("--- QARAF smoke test summary ---")
    ...
    if len(x_steps) > 0 and len(ln_vals) > 0:
        ...
        plot_conv_curvs(
            x=x_steps,
            data_series=[
                {"y": np.asarray(ln_vals, dtype=float), "label": "QARAF", "style": "-", "color": "steelblue"},
            ],
            title="QARAF smoke test: ln(dist_T) vs iterations",
