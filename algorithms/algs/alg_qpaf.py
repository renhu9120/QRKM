...
from algorithms.gradient.grad_qpaf import grad_qpaf
from algorithms.initialization.init_qpaf import init_qpaf
...
from core.hyperparams import QPAF as HP_QPAF
...
def alg_qpaf(
    ...
    *,
    gamma: float = HP_QPAF["gamma"],
    alpha: float = HP_QPAF["alpha"],
    eta: float = HP_QPAF["eta"],
    max_iter: int = 50,
    tol: float = 1e-5,
    eps: float = 1e-12,
) -> Dict[str, Any]:
    """Run QPAF with unified float64 quaternion interface.

    ``y`` must be amplitude measurements ``|a^*x|`` (not intensity ``|a^*x|^2``).
    When ``z0 is None``, uses the native ``init_qpaf`` initializer.

    Returns a dict with ``x_hat``, ``z_path``, ``history["dist_T"]``, and ``config``.
    """
    device = A_mat.device
    if A_mat.dtype != torch.float64:
        raise TypeError("alg_qpaf requires float64 tensors")
    ...
    z_path: List[torch.Tensor] = grad_qpaf(
        A_model,
        y,
        z0,
        T=T,
    ...
    if verbose:
        ...
        for i, z in enumerate(z_iter, start=1):
            log_name = "alg_qpaf_pfe" if use_pfe else "alg_qpaf"
...
def _smoke_test_main() -> None:
    """Run a single synthetic QPAF trial with native init and save convergence curves."""
    d = 50
    n = 10 * d
    T = 200
    seed = 1234
    ...
    convergence_csv_name = "qpaf_smoke_from_alg_convergence.csv"
    curve_pt_name = "qpaf_smoke_from_alg_curve.pt"
    fig_base_name_ln_dist = "qpaf_smoke_from_alg_ln_dist_T"
    ...
    if len(x_steps) > 0 and len(ln_vals) > 0:
        ...
        plot_conv_curvs(
            x=x_steps,
            data_series=[
                {"y": np.asarray(ln_vals, dtype=float), "label": "QPAF", "style": "-", "color": "steelblue"},
            ],
            title="QPAF smoke test: ln(dist_T) vs iterations",
