...
from algorithms.gradient.grad_qrwf import grad_qrwf
...
from core.hyperparams import QRWF as HP_QRWF
...
def alg_qrwf(
    ...
    """Run QRWF with unified float64 quaternion interface.
    ...
    if A_mat.dtype != torch.float64:
        raise TypeError("alg_qrwf requires float64 tensors")
    ...
    z_path: List[torch.Tensor] = grad_qrwf(
        A_model,
        y,  # pass canonical (n,)
        z0,
        eta=float(HP_QRWF["eta"]),
        T=T,
        x_true=x_true,
        stop_err=stop_err,
        use_pfe=use_pfe,
    ...
    if verbose:
        ...
        for i, z in enumerate(z_iter, start=1):
            log_name = "alg_qrwf_pfe" if use_pfe else "alg_qrwf"
...
def _smoke_test_main() -> None:
    """Run a single synthetic QRWF trial and save convergence curves."""
    d = 50
    n = 10 * d
    T = 200
    ...
    convergence_csv_name = "qrwf_smoke_from_alg_convergence.csv"
    curve_pt_name = "qrwf_smoke_from_alg_curve.pt"
    fig_base_name_ln_dist = "qrwf_smoke_from_alg_ln_dist_T"
    ...
    if len(x_steps) > 0 and len(ln_vals) > 0:
        ...
        plot_conv_curvs(
            x=x_steps,
            data_series=[
                {"y": np.asarray(ln_vals, dtype=float), "label": "QRWF", "style": "-", "color": "steelblue"},
            ],
            title="QRWF smoke test: ln(dist_T) vs iterations",
